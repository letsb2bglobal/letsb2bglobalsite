    ...
    <div className="bg-white rounded-[12px] shadow-sm p-5 border border-gray-100 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {author.isFollowing && <span className="text-[10px] font-bold text-[#6B3FA0] bg-purple-50 px-2 py-0.5 rounded">Following</span>}
            </div>
      ...
      {mediaItems && mediaItems.length > 0 ? (
        <div className="flex gap-4 overflow-x-auto pb-4 scroll-smooth snap-x snap-mandatory -mx-2 px-2 scrollbar-thin scrollbar-thumb-purple-200">
          {mediaItems.map((item, idx) => (
            <div key={idx} className="flex-none shrink-0 w-[82%] sm:w-[68%] lg:w-[55%] rounded-[20px] overflow-hidden bg-[#faf8fc] aspect-[16/10] relative group/media border border-purple-50 shadow-sm transition-all hover:shadow-md snap-start">
                {item.type === 'videos' || item.mime?.startsWith('video/') ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center p-6 gap-3">
                    ...
                    <div className="text-center">
                      <p className="text-xs font-black text-gray-900 truncate max-w-[180px]">{item.name || 'document.pdf'}</p>
                      <p className="text-[10px] font-bold text-gray-400 mt-0.5">{item.size ? `${(item.size / 1024).toFixed(1)} KB` : 'PDF Document'}</p>
                    </div>
                    <a 
                      href={item.url} 
                      target="_blank" 
                  ...
                  <img 
                    ...
                    onError={(e) => {
                       (e.target as HTMLImageElement).src = 'https://placehold.co/600x400/f6f2f8/6B3FA0?text=LetsB2B';
                    }}
                   />
                )}
            </div>
      ...
      {details && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-[#fbf9ff] rounded-[20px] border border-purple-50">
          {/* Common Fields */}
          {(type === 'accommodation' || type === 'medical' || type === 'medical_tourism') && details.propertyType && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.propertyType}</span>
            </div>
          )}
          {(type === 'medical' || type === 'medical_tourism') && details.treatmentCategory && (
            <div className="flex flex-col gap-1">
              <span className="flex items-center gap-1.5 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                <Stethoscope size={12} className="text-[#6B3FA0]" /> Treatment
              </span>
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.treatmentCategory}</span>
            </div>
          )}
          {type === 'mice' && details.eventType && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.eventType}</span>
            </div>
          )}
          {type === 'tours' && details.tourType && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.tourType} ({details.tourFormat || 'N/A'})</span>
            </div>
          )}
          {type === 'transportation' && details.serviceType && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.serviceType.replace('_', ' ')}</span>
            </div>
          )}
          {type === 'transportation' && details.vehicleType && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.vehicleType}</span>
            </div>
          )}
          {(type === 'medical' || type === 'medical_tourism') && details.serviceType && (
            <div className="flex flex-col gap-1">
              <span className="flex items-center gap-1.5 text-[9px] font-black text-gray-400 uppercase tracking-widest">
                <HeartPulse size={12} className="text-[#6B3FA0]" /> Service
              </span>
              <span className="text-xs font-black text-gray-800 capitalize leading-none">{details.serviceType}</span>
            </div>
          )}
          ...
          {details.rooms && (
            <div className="flex flex-col gap-1">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Rooms</span>
              <span className="text-xs font-black text-gray-800 leading-none">{details.rooms} Unit(s)</span>
            </div>
          )}
          {details.participants && (
            <div className="flex flex-col gap-1">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Attendance</span>
              <span className="text-xs font-black text-gray-800 leading-none">{details.participants} PAX</span>
            </div>
          )}
          {details.passengers && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 leading-none">{details.passengers} PAX</span>
            </div>
          )}
          {details.patients && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 leading-none">{details.patients} PAX</span>
            </div>
          )}
          {details.attendants && (
            <div className="flex flex-col gap-1">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Attendants</span>
              <span className="text-xs font-black text-gray-800 leading-none">{details.attendants} PAX</span>
            </div>
          )}
          {details.timeline && (
            <div className="flex flex-col gap-1">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Timeline</span>
              <span className="text-xs font-black text-gray-800 leading-none capitalize">{details.timeline}</span>
            </div>
          )}
          {details.luggage && (
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs font-black text-gray-800 leading-none">{details.luggage} Bag(s)</span>
            </div>
          )}
          {details.starCategory && (
            <div className="flex flex-col gap-1">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Category</span>
              <span className="text-xs font-black text-amber-500 leading-none">{details.starCategory} Star</span>
            </div>
          )}
          {details.mealPlan && (
            <div className="flex flex-col gap-1">
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Meal Plan</span>
              <span className="text-xs font-black text-gray-800 leading-none">{details.mealPlan}</span>
            </div>
          )}
        </div>
      )}
      ...
      {(tags?.length || details?.preferences?.length || details?.inclusions?.length || details?.specialInstructions?.length) ? (
        <div className="flex flex-wrap gap-2 pt-2">
          {/* Preferences and Inclusions render as distinct pills */}
          {Array.isArray(details?.preferences) && details.preferences.map((p: string, i: number) => (
             <span key={`p-${i}`} className="px-3 py-1 bg-green-50 text-green-700 text-[9px] font-black uppercase tracking-wider rounded-lg border border-green-100 flex items-center gap-1">
               <CheckCircle2 size={10} /> {p}
             </span>
          ))}
          {Array.isArray(details?.inclusions) && details.inclusions.map((inc: string, i: number) => (
             <span key={`inc-${i}`} className="px-3 py-1 bg-blue-50 text-blue-700 text-[9px] font-black uppercase tracking-wider rounded-lg border border-blue-100 flex items-center gap-1">
               <Briefcase size={10} /> {inc}
             </span>
          ))}
          {Array.isArray(details?.specialInstructions) && details.specialInstructions.map((si: string, i: number) => (
             <span key={`si-${i}`} className="px-3 py-1 bg-amber-50 text-amber-700 text-[9px] font-black uppercase tracking-wider rounded-lg border border-amber-100 flex items-center gap-1">
               <Info size={10} /> {si}
             </span>
          ))}
          {Array.isArray(details?.additionalServices) && details.additionalServices.map((as: string, i: number) => (
             <span key={`as-${i}`} className="px-3 py-1 bg-indigo-50 text-indigo-700 text-[9px] font-black uppercase tracking-wider rounded-lg border border-indigo-100 flex items-center gap-1">
               <Activity size={10} /> {as}
             </span>
          ))}
          {tags && tags.map((tag, i) => (
            <span key={`tag-${i}`} className="px-3 py-1 bg-[#f6f2f8] text-[#6B3FA0] text-[9px] font-black uppercase tracking-wider rounded-lg border border-purple-50">
              #{tag.trim()}
            </span>
          ))}
        </div>
          ...
          {budget && budget.amount > 0 && (
            <div className="flex flex-col border-l border-gray-100 pl-6">
              ...
              <span className="text-sm font-black text-[#6B3FA0]">
                ...
                <span className="text-[10px] text-gray-400 ml-1 font-bold italic">/ {budget.budgetType?.replace('per', '')}</span>
              </span>
            </div>
          )}
        </div>
