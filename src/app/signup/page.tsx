...
import { setAuthData, registerWithOtp, verifyEmailOtp } from '@/lib/auth';
...
export default function SignUpPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      await registerWithOtp(formData.email, formData.username);
      setStep('otp');
    } catch (error: any) {
  ...
  return (
    <AuthLayout>
      ...
      <form 
        ...
        onSubmit={step === 'info' ? handleSendOtp : handleVerifyOtp}
      >
        {step === 'info' ? (
          <>
        ...
        >
          {isLoading 
            ? (step === 'info' ? 'Sending OTP...' : 'Verifying...') 
            : (step === 'info' ? 'REGISTER' : 'VERIFY OTP')
          }
        </button>
      </form>
